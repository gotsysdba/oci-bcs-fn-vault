...
resource "oci_bastion_bastion" "bastion_bastion" {
...
resource "oci_bastion_session" "bastion_session" {
  count      = var.database_demo ? 1 : 0
  bastion_id = oci_bastion_bastion.bastion_bastion[0].id
  ...
  target_resource_details {
    ...
    target_resource_private_ip_address = data.oci_core_private_ips.core_private_ips[0].private_ips[0].ip_address
  }
